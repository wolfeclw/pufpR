...
impute_coords_dist <- function(df, distance_threshold = 100, jitter_amount = 0.00001) {
  ...
  if (nrow(d_lapse) > 0) {
    ...
    if (min_dist > distance_threshold) {
      warning(paste0("The minimum distance between missing coordinates (", min_dist,
      ...
      d_coords_fill <- d_dist %>%
        mutate(impute_lat = ifelse(is.na(lat) & lapse_distance < distance_threshold,
      ...
      d_jitter <- d_jitter %>%
        mutate(jlat = sf::st_coordinates(.)[, 2],
      ...
      d_dist_imputed <- jitter_join %>%
        mutate(imputed_coord = ifelse(!is.na(jlat), 1,
    ...
    message(paste0("The minimum and maximum distance between lapses is ", min_dist, " and ",
...
impute_coords_open <- function(df, distance_threshold = 100, jitter_amount = 0.00001, speed_threshold = 5,
  ...
  d_dist_imputed <- impute_coords_dist(df, distance_threshold = distance_threshold, 
  ...
  speed_f <- function(df) {
    ...
    d_speed <- df %>% 
      ...
      mutate(lag_time.diff = lubridate::ymd_hms(Date_Time) - lag(lubridate::ymd_hms(Date_Time)),
  ...
  if (nrow(h_jitter) > 1 | nrow(t_jitter) > 1) {
    ...
    d_imputed_open <- ht_jitter_join %>% 
      mutate(imputed_coord = ifelse(is.na(imputed_coord) & !is.na(jlat), 1, imputed_coord),
             lat = ifelse(is.na(jlat), lat, jlat),
             lon = ifelse(is.na(jlon), lon, jlon)) %>%
      select(-c(r:length(.)))
  } else {
  ...
  if (speed_threshold < h_speed & !is.infinite(h_speed)) {
    message(paste0("The speed threshold is less than calcualted speed at the head of the file (", h_speed,
                   " m/s) -- the open lapse was not imputed."))
  }
  
  if (speed_threshold < t_speed & !is.infinite(t_speed)) {
    message(paste0("The speed threshold is less than calcualted speed at the tail of the file (", t_speed,
                   " m/s) -- the open lapse was not imputed."))
  }
...
ufp_impute <- function(df, distance_threshold = 100, jitter_amount = 0.00001, speed_threshold = 5, 
  ...
  d_imputed <- if (sum(df$GPS_Valid) == nrow(df)) {
    ...
    impute_coords_open(df, distance_threshold = distance_threshold, jitter_amount = jitter_amount,
  ...
  message(paste0("A total of ", n_imputed , " (", scales::percent(n_imputed/nrow(d_imputed)), ")",
                 " of the coordinates were imputed."))
  message(paste0("A total of ", n_na_coords, " (", scales::percent(n_na_coords/nrow(d_imputed)), ")",
